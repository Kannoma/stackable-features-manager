...
## Dependencies: ModuleAPI, NullSafeAPI
##
...
func _refresh_api_state() -> void:
	...
	if not _real_api and ModuleRegistry._manager:
		if ModuleRegistry._manager.is_module_enabled(_module_name):
			var api = ModuleRegistry._manager.get_module_api(_module_name)
			if api:
				_real_api = api
				_is_enabled = true
