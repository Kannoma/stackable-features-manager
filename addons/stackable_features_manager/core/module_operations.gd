## Module operations coordinator for stackable features [br]
## Coordinates complex operations between different services with consistent error handling
##
...
signal module_imported(success: bool, module_id: String)
signal module_copied(success: bool, source_path: String, dest_path: String)
...
## [code]var ops = ModuleOperations.new(module_manager, git_manager, http_request_node)[/code]
func _init(p_module_manager: ModuleManager, p_git_manager: GitManager, http_request_node: HTTPRequest) -> void:
...
func copy_module_to_destination(source_path: String, dest_path: String) -> Result:
	...
	module_copied.emit(copy_result.is_ok(), source_path, dest_path)
...
func execute_git_integration_with_url(module_id: String, repository_url: String) -> Result:
	...
	var dest_path = git_manager.get_default_module_folder() + "/" + module_id

	# Sync module to repository - handles both new and existing destinations
	var sync_result = git_manager.sync_module_to_repository(source_path, dest_path, repository_url)
	...
	if not git_manager.open_in_git_client(dest_path):
...
func import_module_from_github(repository_url: String) -> Result:
	...
	var module_name = git_manager.extract_module_name_from_url(repository_url)
	var dest_path = stackable_features_path + "/" + module_name
	
	var clone_result = git_manager.clone_repository(repository_url, dest_path)
	if clone_result.is_error():
		module_imported.emit(false, module_id)
		return Result.failure("Failed to clone repository: " + clone_result.get_error())
	
	# Step 6: Refresh module manager and emit success
	module_manager.refresh_available_modules()
	module_imported.emit(true, module_id)
...
func get_module_info_for_git(module_id: String) -> Dictionary:
	...
	return {
		...
		"destination": git_manager.get_default_module_folder() + "/" + module_id
	}
...
func update_module_manifest_repository(module_id: String, repository_url: String) -> Result:
	...
	return git_manager.update_manifest_repository(module_path, repository_url)
